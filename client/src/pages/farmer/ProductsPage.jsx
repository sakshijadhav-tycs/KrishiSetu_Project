...
const ProductsPage = () => {
  ...
  const getProductImage = (images) => {
    if (images && images.length > 0) {
      return resolveImageUrl(images[0], BACKEND_URL);
    }
    return null;
  };
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-3">
        ...
        <div className="flex gap-2">
          <Link
            to="/farmer/deals"
            className="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2 rounded-lg flex items-center space-x-2 transition-colors"
          >
            <FaTags />
      ...
      {filteredProducts.length > 0 ? (
            ...
            <table className="w-full">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="text-center px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider">Price</th>
                  <th className="text-center px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider">Quantity</th>
                  <th className="text-center px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider">Category</th>
                  ...
                  <tr key={product._id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        <div className="w-10 h-10 bg-gray-100 rounded-lg overflow-hidden mr-3">
                          {/* ✅ FIXED: Image src with BASE_URL and fallback logic */}
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-center">
                      <div className={`text-sm font-medium ${
                          product.quantityAvailable === 0 ? "text-red-500" : "text-gray-900"
                        }`}
                      >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex justify-end space-x-3">
                        ...
                        <Link to={`/farmer/products/edit/${product._id}`} className="text-green-600 hover:text-green-900">
                          <FaEdit />
                        </Link>
        ...
        <div className="text-center py-12 bg-white border border-dashed border-gray-300 rounded-xl">
          ...
          <Link to="/farmer/products/add" className="bg-green-600 text-white px-6 py-2 rounded-lg hover:bg-green-700 transition-colors">
